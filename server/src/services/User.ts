      ...
      switch (includeKey) {
        ...
        case 'handicap': {
          groups.push(
            ...[
              'handicap.id',
              'handicap->handicap_type.description',
              'handicap->handicap_weight.description'
            ]
          );

          if (params['filter'] && params['filter'].handicap) {
            whereClauses.push(
              Sequelize.literal('"handicap"."type" = ' + params['filter'].handicap)
            );
          }

          if (params['filter'] && params['filter'].handicap_weight) {
            whereClauses.push(
              Sequelize.literal('"handicap"."weight" = ' + params['filter'].handicap_weight)
            );
          }
          break;
        }
        case 'education': {
          groups.push(...['education.id', 'education->education_type.description']);
          if (params['filter'] && params['filter'].education) {
            whereClauses.push(
              Sequelize.literal('"education"."type" = ' + params['filter'].education)
            );
          }
          break;
        }
        case 'vulnerable': {
          groups.push(...['vulnerable.id', 'vulnerable->vulnerable_type.description']);
          if (params['filter'] && params['filter'].vulnerable) {
            whereClauses.push(
              Sequelize.literal('"vulnerable"."type" = ' + params['filter'].vulnerable)
            );
          }
          break;
        }
        ...
        case 'militery': {
          groups.push(...['militery.id', 'militery->militery_type.description']);
          if (params['filter'] && params['filter'].militery) {
            whereClauses.push(
              Sequelize.literal('"militery"."militery" = ' + params['filter'].militery)
            );
          }
          break;
        }
        case 'company': {
          // groups.push(...['company.id','company.companyId', 'company.start', 'company.end','company->hq.uuid']);
          break;
        }
        case 'age': {
          groups.push(...['age.id']);
          if (params['filter'] && params['filter'].age) {
            let startDOB = new Date();
            startDOB.setFullYear(startDOB.getFullYear() - (params['filter'].age + 10));
            let endDOB = new Date();
            endDOB.setFullYear(endDOB.getFullYear() - params['filter'].age);

            whereClauses.push(
              Sequelize.literal('"age"."dob" > ' + "'" + startDOB.toISOString() + "'")
            );
            whereClauses.push(
              Sequelize.literal('"age"."dob" <= ' + "'" + endDOB.toISOString() + "'")
            );
          }
          break;
        }
        case 'gender': {
          groups.push(...['gender.gender', 'gender->gender_type.description']);
          if (params['filter'] && params['filter'].gender) {
            whereClauses.push(Sequelize.literal('"gender"."gender" = ' + params['filter'].gender));
          }
          break;
        }
        case 'computer': {
          groups.push(...['computer.computer', 'computer->computer_type.description']);
          if (params['filter'] && params['filter'].hasComputer) {
            whereClauses.push(
              Sequelize.literal('"computer"."computer" = ' + params['filter'].hasComputer)
            );
          }
          break;
        }
        case 'computer_certification': {
          groups.push(
            ...[
              'computer_certification.certification',
              'computer_certification->computer_certification_type.description'
            ]
          );
          if (params['filter'] && params['filter'].computer_certification) {
            whereClauses.push(
              Sequelize.literal(
                '"computer_certification"."certification" = ' +
                  params['filter'].computer_certification
              )
            );
          }
          break;
        ...
        case 'mariage': {
          groups.push(...['mariage.id', 'mariage->mariage_type.description']);
          if (params['filter'] && params['filter'].mariage) {
            whereClauses.push(
              Sequelize.literal('"mariage"."mariage" = ' + params['filter'].mariage)
            );
          }
          break;
        }
        case 'training': {
          groups.push(...['training.id', 'training->training_type.description']);
          if (params['filter'] && params['filter'].training) {
            whereClauses.push(
              Sequelize.literal('"training"."training" = ' + params['filter'].training)
            );
          }
          break;
        }
        case 'work': {
          groups.push(...['work.experience', 'work->experience_type.description']);
          if (params['filter'] && params['filter'].work) {
            whereClauses.push(Sequelize.literal('"work"."experience" = ' + params['filter'].work));
          }
          break;
        }
        case 'reputation': {
          groups.push(...['reputation.id']);
          break;
        }
        case 'summary_report': {
          groups.push(...['summary_report.id']);
          break;
        }
        case 'talent_prediction': {
          groups.push(...['talent_prediction.id']);
          break;
        }
        case 'cover_letter': {
          groups.push(...['cover_letter.id']);
          break;
        }
        case 'business_suitability': {
          groups.push(...['business_suitability.id']);
          break;
        }
        case 'careers': {
          groups.push(...['careers.userId']);
          break;
        }
      }
    }
